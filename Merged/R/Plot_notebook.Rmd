...
nine11_demo <- read.csv('../data/df.emo.demographics.csv')
...
```{r sample size}
# 911
# goal: count how many ppl have survey 1 & 2; survey 1 & 3; survey 1 
nine11_long_avg %>% glimpse

# add flag and merge data with demographics
nine11_wide_avg_flag <- nine11_wide_avg %>% mutate(flag2 = !is.na(mean_experienced2), flag3 = !is.na(mean_experienced3), flag4 = !is.na(mean_experienced4)) %>% merge(nine11_demo, by.x = 'Subject', by.y = 'Subject')

nine11_wide_avg_flag$gender %>% table(useNA = 'ifany')
(nine11_wide_avg_flag %>% filter((!flag2) & (!flag3) & (!flag4)))$gender %>% table(useNA = 'ifany')

(nine11_wide_avg_flag %>% filter((flag2) | (flag3) | (flag4)))$gender %>% table(useNA = 'ifany')
(nine11_wide_avg_flag %>% filter((flag2) | (flag3) | (flag4)))$age[(nine11_wide_avg_flag %>% filter((flag2) | (flag3) | (flag4)))$age != 0] %>% mean(na.rm=T)
(nine11_wide_avg_flag %>% filter((flag2) | (flag3) | (flag4)))$age[(nine11_wide_avg_flag %>% filter((flag2) | (flag3) | (flag4)))$age != 0] %>% var(na.rm=T) %>% sqrt

(nine11_wide_avg_flag %>% filter(flag2))$gender %>% table(useNA = 'ifany')
(nine11_wide_avg_flag %>% filter(flag2))$age[(nine11_wide_avg_flag %>% filter(flag2))$age != 0] %>% mean(na.rm=T)
(nine11_wide_avg_flag %>% filter(flag2))$age[(nine11_wide_avg_flag %>% filter(flag2))$age != 0] %>% var(na.rm=T) %>% sqrt

nine11_wide_avg_flag %>% dim
nine11_wide_avg_flag %>% filter(flag2 & flag3 & (!flag4))  %>% dim
nine11_wide_avg_flag %>% filter(flag2 & flag3 & flag4) %>% dim
nine11_wide_avg_flag %>% filter(flag2 & (!flag3) & flag4) %>% dim
nine11_wide_avg_flag %>% filter(flag2 & (!flag3) & (!flag4)) %>% dim
nine11_wide_avg_flag %>% filter((!flag2) & flag3 & (!flag4)) %>% dim
...
# complete survey 2
nine11_wide_avg_flag %>% filter(flag2)  %>% dim
nine11_wide_avg_flag %>% filter(flag3)  %>% dim
nine11_wide_avg_flag %>% filter(flag4)  %>% dim

nine11_wide_avg_flag %>% filter((!flag2) & (!flag3) & (!flag4)) %>% dim
nine11_wide_avg_flag %>% filter((!flag2) & (!flag3) & (!flag4)) %>% view

nine11_wide_avg %>% glimpse
nine11_long_avg %>% filter(time)
nine11_wide %>% dim
nine11_wide %>% filter9
nine11_wide %>% glimpse
nine11_wide$Subject %>% unique %>% length
```
...
```{r fig4}
# Fig 4A
x_range <- seq(1,5,0.25) %>% paste0(collapse = ',')
df_Rem_911_prev <- Rem_911_MCMC_weighted_ref2 %>% get_model_data(type='pred', terms=c(paste0('prev_experience[',x_range,']'),'time')) %>% 
  as.data.frame

df_Rem_911_prev <- df_Rem_911_prev %>% mutate(across(predicted:conf.high,
                             function(x){return(reverse_beta_squeeze(x, length(nine11_BRem$stress_memory), likert_min_max = c(1,5)))},
                             .names = "{.col}_original"))

p_Rem_911_prev <- df_Rem_911_prev %>% ggplot(aes(x=x, y=predicted_original, group=group, color=group)) +
  geom_line()+
  geom_ribbon(aes(ymin=conf.low_original, ymax=conf.high_original,
              fill=group),alpha=0.3, color=NA, show.legend=FALSE)

p_Rem_911_prev + 
  ylab('Remembered Negative Emotions') + 
  xlab('Previous Negative Emotions') + 
  ylim(1,5)+
  scale_color_manual(labels = c('1 year','3 years', '10 years'), values = palette3var)+
  scale_fill_manual(values = palette3var)+
  labs(colour='') + 
  emomem_theme +
  theme(legend.position = 'none')

ggsave('../Output/fig/pdf/Rem_prev.pdf',height=6,width=6)
ggsave('../Output/fig/png/Rem_prev.png',height=6,width=6)
...
```{r}
df_Rem_911_curr <- Rem_911_MCMC_weighted_ref2 %>% get_model_data(type='pred', terms=c(paste0('curr_experience[',x_range,']'),'time')) %>% 
  as.data.frame

df_Rem_911_curr <- df_Rem_911_curr %>% mutate(across(predicted:conf.high,
                             function(x){return(reverse_beta_squeeze(x, length(nine11_BRem$stress_memory), likert_min_max = c(1,5)))},
                             .names = "{.col}_original"))
...
p_Rem_911_curr <- df_Rem_911_curr %>% ggplot(aes(x=x, y=predicted_original, group=group, color=group)) +
  geom_line()+
  geom_ribbon(aes(ymin=conf.low_original, ymax=conf.high_original,
              fill=group),alpha=0.3, color=NA, show.legend=FALSE)
...
p_Rem_911_curr + 
  ylab('Remembered Negative Emotions') + 
  xlab('Current Negative Emotions') + 
  ylim(1,5)+
  scale_color_manual(labels = c('1 year','3 years', '10 years'), values = palette3var)+
  scale_fill_manual(values = palette3var)+
  labs(colour='') + 
...
ggsave('../Output/fig/pdf/Rem_curr.pdf',height=6,width=6)
ggsave('../Output/fig/png/Rem_curr.png',height=6,width=6)
...
ggsave('../Output/fig/pdf/Rem_curr_legend.pdf',height=6,width=6)
ggsave('../Output/fig/png/Rem_curr_legend.png',height=6,width=6)
...
# polygon 
p_ref2 <- mcmc_intervals(as.array(Rem_911_MCMC_weighted_ref2), pars = c('prev_experience','curr_experience'))

p_ref3 <- mcmc_intervals(as.array(Rem_911_MCMC_weighted_ref3), pars = c('prev_experience','curr_experience'))
p_ref4 <- mcmc_intervals(as.array(Rem_911_MCMC_weighted_ref4), pars = c('prev_experience','curr_experience'))

p_ref2_area <- mcmc_areas(as.array(Rem_911_MCMC_weighted_ref2), pars = c('prev_experience','curr_experience'), prob=0.95)

p_ref3_area <- mcmc_areas(as.array(Rem_911_MCMC_weighted_ref3), pars = c('prev_experience','curr_experience'), prob = 0.95)
p_ref4_area <- mcmc_areas(as.array(Rem_911_MCMC_weighted_ref4), pars = c('prev_experience','curr_experience'), prob = 0.95)
...
ggsave('../Output/fig/pdf/ref2_area.pdf',height=2,width=6)
ggsave('../Output/fig/png/ref2_area.png',height=2,width=6)
...
ggsave('../Output/fig/pdf/ref3_area.pdf',height=2,width=6)
ggsave('../Output/fig/png/ref3_area.png',height=2,width=6)
...
ggsave('../Output/fig/pdf/ref4_area.pdf',height=2,width=6)
ggsave('../Output/fig/png/ref4_area.png',height=2,width=6)
...
p_ref4_area + 
  ...
  expand_limits(x=0, y=0) +
  xlab('Coefficient Estimate')
ggsave('../Output/fig/pdf/ref4_area_axis.pdf',height=3,width=6)
ggsave('../Output/fig/png/ref4_area_axis.png',height=3,width=6)
...
```{r}
Rem_911_MCMC_weighted_ref2 %>% describe_posterior()
p_ref2 + emomem_theme
p_ref3 + emomem_theme
p_ref4 + emomem_theme
```
